...
def adsorption_isotherm_1(pressure, temperature, y1, y2):
    """
...
def adsorption_isotherm_2(pressure, temperature, y2, isotherm_type="GAB"):
    """
...
def calculate_gas_heat_capacity():
    return 29.19  # J/mol·K (approx. for air/CO2)


def heat_transfer_coefficient():
    return 3, 26  # W/m²·K (bed, wall)
...
def calculate_axial_dispersion_coefficient(bed_props, inlet_vals):
    D_m = 1.6e-5  # m²/s (molecular diffusion)
...
def energy_balance_error(E, T, P, y1, y2, n1, n2, time, bed_props, grid):
    """
    ...
    heat_gen = (
        ...
        * scipy.integrate.trapezoid(
            np.abs(ΔH1) * (n1[:, -1] - n1[:, 0]) + np.abs(ΔH2) * (n2[:, -1] - n2[:, 0]),
            z,
        )
    )
    ...
    heat_acc_solid = (
        ...
        * Cp_s
        * solid_density
        * scipy.integrate.trapezoid((T[:, -1] - T[:, 0]), z)
    )
    heat_acc_gas = (
        ε
        * A
