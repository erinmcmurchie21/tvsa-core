...
from additional_functions_validation import create_plot
import time
...
temperature_observed = np.loadtxt(
    "pini_data.csv", delimiter=",", usecols=1
)  # Second column
...
def parameter_fitting(parameters, simulation_conditions, T_observed, t_observed):
    ...
    def objective_function(params):
        ...
        time_span = (
            0,
            t_observed[-1],
        )  # Use the last time point from the observed data
    ...
    def ODE_func(t, results_vector):
        return column.ODE_calculations(
            ...
            results_vector=results_vector,
            column_grid=column_grid,
            bed_properties=final_bed_properties,
            inlet_values=inlet_values,
            outlet_values=outlet_values,
        )
...
output = parameter_fitting(
    initial_guess, simulation_conditions, temperature_observed, t_observed
)
