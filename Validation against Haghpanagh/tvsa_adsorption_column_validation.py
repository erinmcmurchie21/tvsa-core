...
def inlet_boundary_conditions(
    ...
    if inlet_values["inlet_type"] == "mass_flow":
        ...
        Pe_h = (
            v_inlet * bed_properties["inner_bed_radius"] / thermal_diffusivity
        )  # Péclet number
...
def ODE_calculations(
    ...
    dn1dt = k1_validation * (
        func.adsorption_isotherm_1(
            P, T, y1, y2, y3, n1, bed_properties, isotherm_type=isotherm_type_1
        )[0]
        - n1
    )  # mol / m3
    ...
    # Heat capacity terms
    q1 = n1 * bed_properties["bed_density"] / (1 - bed_properties["bed_voidage"])
    q2 = n2 * bed_properties["bed_density"] / (1 - bed_properties["bed_voidage"])

    a_2 = (1 - bed_properties["bed_voidage"]) / bed_properties["bed_voidage"] * (
        Cp_solid * bed_properties["bed_density"] + Cp_1 * n1 + Cp_2 * n2
    ...
    heat_transfer_term = (
        ...
        * h_bed
        * (T - Tw)
        / (bed_properties["bed_voidage"] * bed_properties["inner_bed_radius"])
    )
    ...
    dE3dt = scipy.integrate.trapezoid(
        bed_properties["wall_density"] * bed_properties["wall_heat_capacity"] * dTwdt,
        column_grid["xCentres"][1:-1],
    )
