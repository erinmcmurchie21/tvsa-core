...
def inlet_boundary_conditions(
    ...
    if inlet_values["inlet_type"] == "mass_flow":
        ...
        ) / (1 + v_inlet / D_l * column_grid["xCentres"][column_grid["nGhost"]])
        y2_inlet = (
            y2[0]
            + v_inlet
            / D_l
        ...
        ) / (1 + column_grid["xCentres"][column_grid["nGhost"]] * v_inlet * Pe_h)
...
def calculate_internal_wall_values(
    ...
    dTdz_walls[0] = -(v_inlet * Pe_h) * (
        inlet_values["feed_temperature"] - T_inlet
...
def ODE_calculations(
    ...
    k1_validation = (
        ...
        * y1
        / func.adsorption_isotherm_1(
            P, T, y1, y2, y3, n1, bed_properties, isotherm_type_1=isotherm_type_1
        )[0]
        * 15
        * bed_properties["particle_voidage"]
        * bed_properties["molecular_diffusivity"]
        / (
            bed_properties["tortuosity"]
            * (bed_properties["particle_diameter"] / 2) ** 2
        )
    )
    ...
    k2_validation = (
        ...
        * y2
        / func.adsorption_isotherm_2(
            P, T, y1, y2, bed_properties, isotherm_type_2=isotherm_type_2
        )[0]
        * 15
        * bed_properties["particle_voidage"]
        * bed_properties["molecular_diffusivity"]
    ...
    dy1dz = (y1_all[1 : num_cells + 2] - y1_all[0 : num_cells + 1]) / (
        column_grid["xCentres"][1 : num_cells + 2]
        - column_grid["xCentres"][0 : num_cells + 1]
    )
    dy2dz = (y2_all[1 : num_cells + 2] - y2_all[0 : num_cells + 1]) / (
        column_grid["xCentres"][1 : num_cells + 2]
        - column_grid["xCentres"][0 : num_cells + 1]
    )
    dy3dz = (y3_all[1 : num_cells + 2] - y3_all[0 : num_cells + 1]) / (
        column_grid["xCentres"][1 : num_cells + 2]
        - column_grid["xCentres"][0 : num_cells + 1]
    )
    ...
    dispersion_term = +(
        ...
        * (
            ...
            * (
                P_walls[1 : num_cells + 1]
                * (
                    Cp_1 * dy1dz[1 : num_cells + 1]
                    + Cp_2 * dy2dz[1 : num_cells + 1]
                    + Cp_3 * dy3dz[1 : num_cells + 1]
                    + Cp_4 * dy4dz[1 : num_cells + 1]
                )
                - P_walls[:num_cells]
                * (
                    Cp_1 * dy1dz[:num_cells]
                    + Cp_2 * dy2dz[:num_cells]
                    + Cp_3 * dy3dz[:num_cells]
                    + Cp_4 * dy4dz[:num_cells]
                )
            )
        )
    )

    adsorbent_heat_generation = +(1 - bed_properties["bed_voidage"]) * (
        np.abs(deltaH_1) * dn1dt + np.abs(deltaH_2) * dn2dt
    )

    accumulation_term = -(
        (1 - bed_properties["bed_voidage"]) * bed_properties["R"] * T * (dn1dt + dn2dt)
        + bed_properties["bed_voidage"]
        * T
        / column_grid["deltaZ"][1:-1]
        * (
    ...
    adsorption_term = (
        -T
        * bed_properties["R"]
        * (1 - bed_properties["bed_voidage"] / bed_properties["total_voidage"])
        * (dn1dt + dn2dt)
    )
    convective_term = (
        -T
