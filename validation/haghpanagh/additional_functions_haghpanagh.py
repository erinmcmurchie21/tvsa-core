...
def adsorption_isotherm_1(
    pressure, temperature, y1, y2, y3, n1, bed_properties, isotherm_type_1="Haghpanagh"
):
    """
    ...
    if isotherm_type_1 == "Toth":
        ...
        load_kg = qs1 * b1 * c1 / (1 + b1 * c1 + b2 * c2) + qs2 * d1 * c1 / (
            1 + d1 * c1 + d2 * c2
        )  # mol/kg
...
def adsorption_isotherm_2(
    pressure, temperature, y1, y2, bed_properties, isotherm_type_2="Haghpanagh"
):
    """
    ...
    if isotherm_type_2 == "GAB":
        ...
        load_kg = qsb * b2 * c2 / (1 + b1 * c1 + b2 * c2) + qsd * d2 * c2 / (
            1 + d1 * c1 + d2 * c2
        )  # mol/kg
...
def energy_balance_error(E, T, P, y1, y2, y3, n1, n2, Tw, time, bed_props, grid):
    ...
    heat_acc_solid = (
        A * Cp_s * bed_density * np.sum((T[:, -1] - T[:, 0]) * grid["deltaZ"][1:-1])
    )
    heat_acc_gas = (
        ε
        * A
