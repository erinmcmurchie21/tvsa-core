...
def pressure_ramp(t, stage, ramp_duration=2.0):
    ...
    if stage == "blowdown":
        ...
        if t <= ramp_duration:
            ...
            pressure = initial_pressure - (initial_pressure - final_pressure) * (
                t / ramp_duration
            )
        else:
        ...
        cooling_ramp_duration = max(
            ramp_duration * 10, 50.0
        )  # At least 50 seconds for cooling

        if t <= cooling_ramp_duration:
            # Linear ramp over cooling_ramp_duration
            pressure = initial_pressure + (final_pressure - initial_pressure) * (
                t / cooling_ramp_duration
            )
        else:
            pressure = final_pressure
...
def run_stage(
    left_values, right_values, column_direction, stage, t_span, initial_conditions
):
    t0 = time.time()
...
def run_cycle(n_cycles):
    ...
    current_initial_conditions = (
        initial_conditions  # Start with global initial conditions
    )
    ...
    for cycle in range(n_cycles):
        ...
        left_values, right_values, column_direction, stage = define_boundary_conditions(
            "adsorption", bed_properties
        )
        t_span = [0, 1000]  # Time span for the ODE solver
        final_conditions_adsorption, _, _, _, _, _, _ = run_stage(
            left_values,
            right_values,
            column_direction,
            stage,
        ...
        (
            ...
            stage,
        ) = define_boundary_conditions("blowdown", bed_properties)
        t_span_blowdown = [0, 100]  # Time span for the ODE solver
        final_conditions_blowdown, _, _, _, _, _, _ = run_stage(
            left_values_blowdown,
            right_values_blowdown,
            column_direction_blowdown,
            stage,
        ...
        left_values_heating, right_values_heating, column_direction_heating, stage = (
            define_boundary_conditions("heating", bed_properties)
        )
        final_conditions_blowdown[
            2 * column_grid["num_cells"] : 3 * column_grid["num_cells"]
        ] = 400 / bed_properties["T_ref"]  # Set wall temperature to 400 K
        t_span_heating = [0, 500]  # Time span for the ODE solver
        final_conditions_heating, _, _, _, _, _, _ = run_stage(
            left_values_heating,
        ...
        left_values_cooling, right_values_cooling, column_direction_cooling, stage = (
            define_boundary_conditions("cooling", bed_properties)
        )
        t_span_cooling = [0, 2000]  # Time span for the ODE solver
        final_conditions_cooling, _, _, _, _, _, _ = run_stage(
            left_values_cooling,
...
def main():
    ...
    (
        ...
        final_outlet_CO2_profile,
        final_time_profile,
        final_pressure_profile,
        final_adsorbed_CO2_profile,
        final_wall_temperature_profile,
    ) = run_cycle(5)
    # Run 5 cycles
