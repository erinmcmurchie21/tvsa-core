...
def create_fixed_properties():
    ...
    bed_properties = {
        ...
        "stage_config": {
            ...
            "pressurisation": {"left": "inlet_pressure", "right": "closed", "direction": "forwards"},
        }
    ...
    y2_feed = relative_humidity_to_mole_fraction(0.55, 
                        bed_properties["ambient_pressure"], bed_properties["ambient_temperature"])
    ...
    n1_init = adsorption_isotherm_1(P_init, T_init, y1_init, y2_init, y3_init, 400e-6, y2_init[0],
    ...
    print(f"Initial adsorbed CO2 loading (mid-bed): {q1_init[num_cells//2]:.2f} mol/m³")
...
def adsorption_isotherm_1(pressure, temperature, y1, y2, y3, n1, n2, bed_properties, isotherm_type_1="WADST"):
    ...
    if isotherm_type_1 == "Toth":
        ...
        load_kg = n_s * b * pressure * y1 / (1 + (b * pressure * y1)**t)**(1 / t)  # mol/kg
        ...
        n_s = 4.86 * np.exp(0 * (1 - temperature / T_0)) * (1/(1-gamma*n2_mass))       # mol/kg
        b = 28.5 * np.exp(-1*(-117798) / (R * T_0) * (1-T_0 / temperature)) * (1 + beta *n2_mass) # kPa⁻¹
        t = 0.209 - 0.523 * (1 - T_0 / temperature)
        ...
        load_kg = ((1-np.exp(-A/n2_mass)) * n_s_dry * b_dry * pressure * y1 / (1 + (b_dry * pressure * y1)**t_dry)**(1 / t_dry)
                   + np.exp(-A/n2_mass) * n_s_wet * b_wet * pressure * y1 / (1 + (b_wet * pressure * y1)**t_wet)**(1 / t_wet))  # mol/kg
        load_m3 = load_kg * bed_density / (1 - ε)  # mol/m³
        ΔH = -70000  # J/mol
        ...
        f = n_s * b * pressure * y1 / (1 + (b * pressure * y1)**t)**(1 / t)  # mol/kg
