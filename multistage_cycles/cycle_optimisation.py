    ...
    try:
        for number in range(int(n_cycles)):
            for stage_name, t_span in stages:
                ...
                left_values, right_values, column_direction, stage = cycle.define_boundary_conditions(
                    stage_name, bed_properties, P_left, P_right)

                # Run stage with all required parameters explicitly passed
                stage_results = cycle.run_stage(left_values, right_values, column_direction, stage_name, t_span, 
                                                current_initial_conditions, cycle_properties, solver='BDF')
...
def main():
    ...
    # Run for baseline
    baseline_result = run_cycle_with_params(baseline_durations, desorption_temp,
                                    vacuum_pressure, n_cycles=1)
    ...
    print(f"Baseline net CO2 (kg)={co2_norm:.3e}, baseline energy (J)={energy_norm:.3e}")
    ...
    result = differential_evolution(objective_function, bounds, maxiter=10, popsize=5, polish=True, seed=42, workers=1)
    end_time = time.time()
    ...
    best_params = result.x  # Note: result.x not result.params for differential_evolution
    best_durations = {
    ...
    best_sim = run_cycle_with_params(best_durations, desorption_temp=best_T, 
                                   vacuum_pressure=best_Pvac, n_cycles=1)
