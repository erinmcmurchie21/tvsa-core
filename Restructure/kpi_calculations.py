...
def calculate_cycle_kpis(
    ...
    total_simulation_time = sum(
        stage.simulation_time for stage in stage_kpis_dict.values()
    )
    MW_CO2 = properties.fluid.MW_CO2
    ...
    if desorption_stage:
        total_carrier_gas = (
            desorption_stage.N2_out + desorption_stage.O2_out + desorption_stage.H2O_out
        )
    ...
    if total_duration > 0:
        productivity = (
            total_CO2_captured / properties.geometry.bed_volume / (total_duration)
        )
    ...
    sorbent_mass = properties.adsorbent.sorbent_mass  # kg
    ...
    specific_mechanical_energy = (
        (total_fan_energy + total_vacuum_energy) / 1e6 / kg_CO2_captured
    )  # MJ/kg
    total_specific_energy = (
        specific_mechanical_energy
        + properties.efficiency.compressor_efficiency
        * (
            1
            - properties.operation.ambient_temperature
            / properties.operation.desorption_temperature
        )
...
def print_cycle_kpis(cycle_kpis: CycleKPIs):
    ...
    print(
        f"Total Simulation Time: {cycle_kpis.total_simulation_time:.2f} s ({cycle_kpis.total_simulation_time / 60:.2f} min)"
    )
    ...
    print(f"{'─' * 60}")
    print(
        f"Specific Thermal Energy:   {cycle_kpis.specific_thermal_energy:.2e} MJ/kgCO2"
    )
    print(
        f"Specific Mechanical Energy: {cycle_kpis.specific_mechanical_energy:.2e} MJ/kgCO2"
    )
    print(
        f"Total Specific Energy:      {cycle_kpis.total_specific_energy:.2e} MJ/kgCO2"
    )
