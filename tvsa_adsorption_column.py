...
import additional_functions_clean as func
...
def inlet_boundary_conditions(
    ...
    if inlet_values["inlet_type"] == "mass_flow":
        ...
        D_l = func.calculate_axial_dispersion_coefficient(
            bed_properties, inlet_values
        )  # [m²/s]
        v_inlet = inlet_values["velocity"]  # [m/s]
        Cp_g = func.calculate_gas_heat_capacity()  # [J/mol·K]
        thermal_diffusivity = func.calculate_gas_thermal_conductivity() / (
            Cp_g * rho_gas_inlet
        )  # [m²/s]
        ...
        avg_density_inlet = (
            ...
            * (
                ...
                + bed_properties["MW_4"] * (1 - y1_inlet - y2_inlet - y3_inlet)
            )
        )  # [kg/m³]
        ...
        Pe_h = (
            v_inlet * bed_properties["inner_bed_diameter"] / thermal_diffusivity
        )  # Péclet number
...
def outlet_boundary_conditions(
    ...
    if outlet_values["outlet_type"] == "pressure":
        ...
        b = (
            150
            * mu
            * (1 - bed_properties["bed_voidage"]) ** 2
            / (
                bed_properties["bed_voidage"] ** 3
                * bed_properties["particle_diameter"] ** 2
            )
...
def calculate_internal_wall_values(
    ...
    r_r1 = (
        ((y1_all[1 : Nx + 1] - y1_all[:Nx]) + epsilon)
        / ((y1_all[2 : Nx + 2] - y1_all[1 : Nx + 1]) + epsilon)
        * (column_grid["deltaZ"][2 : Nx + 2] + column_grid["deltaZ"][1 : Nx + 1])
    ...
    y1_walls = y1_all[1 : Nx + 1] + flux_limiter1 * (
        y1_all[2 : Nx + 2] - y1_all[1 : Nx + 1]
    )
    ...
    r_r2 = (
        ((y2_all[1 : Nx + 1] - y2_all[:Nx]) + epsilon)
        / ((y2_all[2 : Nx + 2] - y2_all[1 : Nx + 1]) + epsilon)
        * (column_grid["deltaZ"][2 : Nx + 2] + column_grid["deltaZ"][1 : Nx + 1])
        / (column_grid["deltaZ"][1 : Nx + 1] + column_grid["deltaZ"][0:Nx])
    )
    modified_van_leer2 = (0.5 * R_r * r_r2 + 0.5 * R_r * abs(r_r2)) / (R_r + r_r2 - 1)
    flux_limiter2 = modified_van_leer2 / R_r
    y2_walls = y2_all[1 : Nx + 1] + flux_limiter2 * (
        y2_all[2 : Nx + 2] - y2_all[1 : Nx + 1]
    )
    ...
    r_r3 = (
        ((y3_all[1 : Nx + 1] - y3_all[:Nx]) + epsilon)
        / ((y3_all[2 : Nx + 2] - y3_all[1 : Nx + 1]) + epsilon)
        * (column_grid["deltaZ"][2 : Nx + 2] + column_grid["deltaZ"][1 : Nx + 1])
        / (column_grid["deltaZ"][1 : Nx + 1] + column_grid["deltaZ"][0:Nx])
    )
    modified_van_leer3 = (0.5 * R_r * r_r3 + 0.5 * R_r * abs(r_r3)) / (R_r + r_r3 - 1)
    flux_limiter3 = modified_van_leer3 / R_r
    y3_walls = y3_all[1 : Nx + 1] + flux_limiter3 * (
        y3_all[2 : Nx + 2] - y3_all[1 : Nx + 1]
    )
    y3_walls[-1] = y3_outlet
    y3_walls = np.concatenate((np.array([y3_inlet]), y3_walls))

    r_r_T = (
        ((T_all[1 : Nx + 1] - T_all[:Nx]) + epsilon)
        / ((T_all[2 : Nx + 2] - T_all[1 : Nx + 1]) + epsilon)
        * (column_grid["deltaZ"][2 : Nx + 2] + column_grid["deltaZ"][1 : Nx + 1])
        / (column_grid["deltaZ"][1 : Nx + 1] + column_grid["deltaZ"][0:Nx])
    )
    modified_van_leer_T = (0.5 * R_r * r_r_T + 0.5 * R_r * abs(r_r_T)) / (
        R_r + r_r_T - 1
    )
    flux_limiter_T = modified_van_leer_T / R_r
    T_walls = T_all[1 : Nx + 1] + flux_limiter_T * (
        T_all[2 : Nx + 2] - T_all[1 : Nx + 1]
    )
    T_walls[-1] = T_outlet
    ...
    rho_gas_walls = (
        P_walls / bed_properties["R"] / T_walls
    )  # Assuming ideal gas law for density calculation
    avg_density_walls = (
        rho_gas_walls
        / 1000
        * (
            bed_properties["MW_1"] * y1_walls
            + bed_properties["MW_2"] * (y2_walls)
            + bed_properties["MW_3"] * (y3_walls)
            + bed_properties["MW_4"] * (1 - y1_walls - y2_walls - y3_walls)
        )
    )  # Factor of 1000 to convert from g/mol to kg/mol
    mu = func.calculate_gas_viscosity()
    a = (
        1.75
        * (1 - bed_properties["bed_voidage"])
...
def ODE_calculations(
    t, results_vector, column_grid, bed_properties, inlet_values, outlet_values
):
    """
    ...
    k1 = 0.0002  # * bed_properties["rho_bed"] / (1 - bed_properties["bed_voidage"])
    k2 = 0.002  # * bed_properties["rho_bed"] / (1 - bed_properties["bed_voidage"])

    # Solid phase balance for adsorbed components
    # ∂q₁/∂t = k₁(q₁* - q₁)
    dn1dt = k1 * (func.adsorption_isotherm_1(P, T, y1, y2)[0] - n1)  # mol / m3
    deltaH_1 = func.adsorption_isotherm_1(P, T, y1, y2)[1]  # Heat of adsorption (J/mol)

    # ∂q₂/∂t = k₂(q₂* - q₂)
    dn2dt = k2 * (func.adsorption_isotherm_2(P, T, y2)[0] - n2)
    deltaH_2 = func.adsorption_isotherm_2(P, T, y2)[1]  # Heat of adsorption (J/mol)
    ...
    a_2 = (1 - bed_properties["bed_voidage"]) / bed_properties["bed_voidage"] * (
        Cp_solid * bed_properties["sorbent_density"] + Cp_1 * n1 + Cp_2 * n2
    ) + Cp_g * P / (bed_properties["R"] * T)
    ...
    convection_term = (
        -Cp_g
        / (bed_properties["R"] * column_grid["deltaZ"][1:-1])
        * (
    ...
    accumulation_term = (
        ...
        * (
            ...
            * (dn1dt + dn2dt)
            - T
            / (column_grid["deltaZ"][1:-1] * bed_properties["bed_voidage"])
            * (
    ...
    heat_transfer_term = (
        ...
        * h_bed
        * (T - Tw)
        / (bed_properties["bed_voidage"] * bed_properties["inner_bed_diameter"])
    )
    ...
    bed_heat_exchange = (
        2
        * bed_properties["inner_bed_diameter"]
        * h_bed
        * (T - Tw)
        / (
            bed_properties["outer_bed_diameter"] ** 2
            - bed_properties["inner_bed_diameter"] ** 2
        )
    )
    ambient_heat_loss = (
        -2
        * bed_properties["outer_bed_diameter"]
        * h_wall
    ...
    derivatives = np.concatenate(
        [dPdt, dTdt, dTwdt, dy1dt, dy2dt, dy3dt, dn1dt, dn2dt, dFdt, dEdt]
    )
