...
from additional_functions_validation import create_non_uniform_grid, adsorption_isotherm_1, adsorption_isotherm_2, total_mass_balance_error, CO2_mass_balance_error, energy_balance_error, create_plot, create_combined_plot, interpolator
import time
...
# Import data
time_observed_temperature = np.loadtxt('data/pini_data_temperature.csv', delimiter=',', usecols=0)  # First column
temperature_observed = np.loadtxt('data/pini_data_temperature.csv', delimiter=',', usecols=1)  # Second column
temperature_observed += 273.15  # Adjusting temperature values by adding 273.15

time_observed_molefraction = np.loadtxt('data/pini_data_molefraction.csv', delimiter=',', usecols=0)  # First column
molefraction_observed = np.loadtxt('data/pini_data_molefraction.csv', delimiter=',', usecols=1)  # Second column
...
def plot_results(simulation_conditions, observed_data, best_params):
    ...
    T_sim_fitted = output_matrix_fitted.y[column_grid["num_cells"]:2*column_grid["num_cells"]] * bed_properties["T_ref"]
    ...
    axes[0].plot(output_matrix_fitted.t, T_sim_fitted[9, :], 
                        label="Fitted (h_bed ="+ str(best_params["h_bed"]) + ", h_wall = " + str(best_params["h_wall"]) + ")", linewidth=2, linestyle='-')
...
def global_objective_function(parameters, initial_guess, simulation_conditions, observed_data, param_names, weights, tot_time, optimization_history):
     # Create a copy of bed_properties to avoid modifying the original
        bed_properties = simulation_conditions["bed_properties"].copy()
        ...
        def ODE_func(t, results_vector):
            return column.ODE_calculations(t, results_vector=results_vector, 
                                            ...
                                            bed_properties=bed_properties, 
                                            left_values=left_values, 
                                            right_values=right_values, column_direction=column_direction)
        ...
        if len(optimization_history) % 1 == 0:
            print(f"Iteration {len(optimization_history)}: Error = {total_error:.6e}, Parameters = {parameters}")
    
        return total_error

def objective_for_DE(params):
    global global_param_names, global_simulation_conditions, global_observed_data, global_weights, global_tot_time, global_optimization_history, global_initial_guess, global_bounds
    return global_objective_function(params, global_initial_guess, global_simulation_conditions, global_observed_data, global_param_names, global_weights, global_tot_time, global_optimization_history)
