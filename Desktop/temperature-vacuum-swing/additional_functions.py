...
def adsorption_isotherm_1(pressure, temperature, mole_fraction_1, mole_fraction_2):
    ...
    equilibrium_loading_per_kg = n_s*b*pressure*mole_fraction_1 / (1+(b*pressure*mole_fraction_1)**t)**(1/t) # mol/kg
    equilibrium_loading = equilibrium_loading_per_kg * sorbent_density / (1-bed_voidage)  # Convert to mol/m³
...
def adsorption_isotherm_2(pressure, temperature, mole_fraction_2, isotherm_type="GAB"):
    ...
    if isotherm_type == "GAB":
        ...
        c_m = 36.48 #mol/kg
        c_G = 0.1489
        saturation_pressure = 6.1094 * np.exp(17.625 * (temperature - 273.15) / ((temperature-273.15) + 243.04))  # hPa
        RH = mole_fraction_2*pressure/(saturation_pressure*100) # Relative humidity
    
        equilibrium_loading_per_kg = c_m * c_G * K_ads * RH / ((1 - K_ads * RH)*(1+(c_G-1)*K_ads*RH)) # mol/kg
        # mol/kg * kg/m3 --> mol/m3
        equilibrium_loading = equilibrium_loading_per_kg * sorbent_density / (1-bed_voidage) # Convert to mol/m³
...
def total_mass_balance_error(F_result, P_result, T_result, n1_result, n2_result, time, bed_properties, column_grid):
    ...
    mole_in = scipy.integrate.trapezoid(F_result[0, :] + F_result[1, :] + F_result[2, :] + F_result[3, :], time)
    mole_out = scipy.integrate.trapezoid(F_result[4, :] + F_result[5, :] + F_result[6, :] + F_result[7, :], time)
    ...
    final_moles = scipy.integrate.trapezoid(
        (bed_voidage * column_area * P_result[:, -1] / (R * T_result[:, -1])
         + (1 - bed_voidage) * column_area * (n1_result[:, -1] + n2_result[:, -1]), z))

    initial_moles = scipy.integrate.trapezoid(
        (bed_voidage * column_area * P_result[:, 0] / (R * T_result[:, 0])
         + (1 - bed_voidage) * column_area * (n1_result[:, 0] + n2_result[:, 0])), z)
...
def CO2_mass_balance_error(F_result, P_result, T_result, y1_result, n1_result, time, bed_properties, column_grid):
    bed_voidage = bed_properties["bed_voidage"]
    column_area = bed_properties["column_area"]
    R = bed_properties["R"]
    z = column_grid["xCentres"][1:-1]  # Use correct key name
    ...
    mole_out = scipy.integrate.trapezoid(F_result[4, :], time)  # Assuming F[1] is outlet flow
...
def energy_balance_error(E_result, T_result, P_result, y1_result, y2_result, n1_result, n2_result, time, bed_properties, column_grid):
    ...
    deltaH_1 = adsorption_isotherm_1(P_result[:, -1], T_result[:, -1], y1_result[:, -1], y2_result[:, -1])[1]  # Convert kJ/mol to J/mol
    deltaH_2 = adsorption_isotherm_2(P_result[:, -1], T_result[:, -1], y2_result[:, -1])[1]  # Convert kJ/mol to J/mol
